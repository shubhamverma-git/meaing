...
export default function Strategy({ trendingData, blog, data, seoData, slug }) {
  ...
  return (
    <>
      ...
      <div className={styles.service_section_container}>
        ...
        />
        {attributes.software.data.length > 0 && (
...
export async function getStaticProps() {
  const result = await getServiceDetailPageData("strategy");
  const ideas = await getHighlightedArticles("idea", "strategy", 1, 3);
  const blog = await getHighlightedArticles("blog", "strategy", 1, 100);
  const seoData = await getSeoData("strategy");
  return {
    props: {
