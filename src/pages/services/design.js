...
export default function design({ trendingData, blog, data, seoData, slug }) {
...
export async function getStaticProps() {
  const result = await getServiceDetailPageData("design");
  const ideas = await getHighlightedArticles("idea", "design", 1, 3);
  const blog = await getHighlightedArticles("blog", "design", 1, 100);
  const seoData = await getSeoData("design");
  return {
    props: {
