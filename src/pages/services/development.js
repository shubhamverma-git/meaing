...
export async function getStaticProps() {
  const result = await getServiceDetailPageData("development");
  const ideas = await getHighlightedArticles("idea", "development", 1, 3);
  const blog = await getHighlightedArticles("blog", "development", 1, 100);
  const seoData = await getSeoData("development");
  return {
    props: {
